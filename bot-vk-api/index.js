...
const token = '7d045b829c64ffc84fe94b575afe40ab779148ed050fb81c1ee1c942d3641947cc0c74eada0068dde7df1';
let AppDataPath = './src/data'
const accesTk = 'vk1.a.h7UsY62CIcMdEqaS7AhQCoBl5CtIVO1wR8C_0Qw13TuhMj2gYlEmtchVgAT7-akIMDVaKu40XLY_fiBb5nVRCn_GpHw1WltrRG-LPEtpLjhpKZuikbCDUG2awzY-YE90ojE5pDJxSWlAG2IZfg_e_VNEO0LljrGPOJ6lbwNssUMhq1JAGh79anJaOUiZsvsm'
const universalGroupId = "208081995"

//------------------------------------DB SERVICE------------------------------------//
var db = new sqlite3.Database('./src/db/groupPost.db', (err) => {
    if (err) {
        return console.log(err.message);
    }
    console.log("Connected to database")
});
db.run('CREATE TABLE IF NOT EXISTS wall_post(id INTEGER primary key, query_text, id_group, name_group, members, checkbox)');
db.run('CREATE TABLE IF NOT EXISTS post(id INTEGER primary key, id_group REFERENCES wall_post(id_group), search_name, photo, post)');

// Для select запросов к бд. Возвращает массив []
async function getdata(sql, row) {
    return await new Promise((callback) => {
        try {
            ...
            return callback('Ошибка запроса! sql: ' + sql + '\n>>>>' + e)
        }
    })
}
// Для insert, update и других запросов к бд, возвращает ID записи
...
//------------------------------------VK SERVICE------------------------------------//
async function WallPost(gid, ptxt) {
    let GetData = fs.readFileSync('./src/data/post_data.json', 'utf8')
    GetData = JSON.parse(GetData)

    let old_time = new Date(GetData.date_last_itr)
    
   

    let new_time = new Date()
    
    let calc_time = (new_time.getTime() - old_time.getTime()) / (1000 * 60)
    let timestamp = (new_time.getTime() - old_time.getTime()) / (1000)
    console.log(calc_time)
    if (calc_time > 1440) {
        GetData.iteration_number = 0
        GetData.date_last_itr = new Date()
        fs.writeFileSync(AppDataPath + '/post_data.json', JSON.stringify(GetData), 'utf-8')
        console.log('Таймер сброшен')
    }
    if (GetData.iteration_number <= 190) {
    //Читаем JSON-файл и сразу парсим его
        let resultPhoto = fs.readFileSync(AppDataPath + "/save.json", "utf8")
        resultPhoto = JSON.parse(resultPhoto)
        console.log("Нажал")
        console.log("Owner: " + resultPhoto.response[0].owner_id)
        console.log("id: " + resultPhoto.response[0].id)
